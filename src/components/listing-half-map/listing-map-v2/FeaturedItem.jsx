...
const FeaturedItem = () => {
  ...
  const keywordHandler = (item) => {
    return item.title[lang].toLowerCase().includes(keyword?.toLowerCase());
  };
  // location handler
  ...
  const statusHandler = (item) => {
    return item.type[lang].toLowerCase().includes(status.toLowerCase());
  };

  // properties handler
  const propertiesHandler = (item) => {
    return item.type[lang].toLowerCase().includes(propertyType.toLowerCase());
  };

  // price handler
  const priceHandler = (item) => {
    return item.price[lang] < price?.max && item.price > price?.min;
  };
...
building
  // status handler
  let content = properties?.slice(0, 8).map((item) => (
    <div
      className={`${
